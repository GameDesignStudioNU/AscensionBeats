...
public class CharacterController2D : MonoBehaviour
{
    ...
    public float BoxHorOffset;
    public float BoxVertOffset;
    ...
    public bool CanWallJump { get
        {
            var boxCastHit = Physics2D.OverlapBox(new Vector2(_transform.position.x + BoxHorOffset, _transform.position.y + BoxVertOffset), new Vector2(BoxWidth, BoxHeight), 0f, PlatformMask);
            if (boxCastHit)
            {
                if (boxCastHit.transform.position.x > _transform.position.x)
                    lastWallTouchRight = true;
                else
                    lastWallTouchRight = false;
            }

            return !State.IsGrounded && !State.IsDashing && boxCastHit && boxCastHit.gameObject.tag != "Border";
        } }
    public bool CanCling { get
        {
            var boxCastHit = Physics2D.OverlapBox(new Vector2(_transform.position.x + BoxHorOffset, _transform.position.y + BoxVertOffset), new Vector2(BoxWidth, BoxHeight), 0f, PlatformMask);
            if (boxCastHit)
            {
            ...
            return !State.IsDashing && boxCastHit && boxCastHit.gameObject.tag != "Border";
        } }
    ...
    private float _boxColliderLocalScaleX;
    ...
    public bool lastWallTouchRight;
    ...
    public void Awake()
    {
        ...
        _boxColliderLocalScaleX = _boxCollider.transform.localScale.x;
    ...
    public void FixedUpdate()
    {
        ...
        if (State.DashDuration > 0)
            State.DashDuration -= Time.fixedDeltaTime;

        if (!State.IsJumping && !State.IsWallJumping && State.DashDuration <= 0)
        {
        ...
        // Move!
        Move(Velocity * Time.deltaTime * not_paused);
    ...
    public void Jump()
    {
        ...
        if (_jumpIn < 0 && State.DashDuration <= 0)
        {
            SetForceVertical(0);
    ...
    public void ClingCheck()
    {
        if (State.IsClinging)
        {
            ...
            _boxCollider.transform.localScale = new Vector3 (Mathf.Sign(_boxCollider.transform.localScale.x) * _boxColliderLocalScaleX * .8f, _boxCollider.transform.localScale.y, _boxCollider.transform.localScale.z);
        ...
        {
            _boxCollider.transform.localScale = new Vector3(Mathf.Sign(_boxCollider.transform.localScale.x) * _boxColliderLocalScaleX, _boxCollider.transform.localScale.y, _boxCollider.transform.localScale.z);
        }
    }
    ...
    public void ClimbCheck()
    {
        ...
        if (State.IsClinging)
        {
            ...
            {
                SetForceVertical(Mathf.Lerp(_velocity.y, Parameters.ClimbVelocity, Time.deltaTime * 20f));

                var boxCastHit = Physics2D.OverlapBox(new Vector2(_transform.position.x, _transform.position.y - .6f), new Vector2(1.2f, .2f), 0f, PlatformMask);
                if (!boxCastHit)
                {
                    SetForceVertical(Mathf.Lerp(_velocity.y, Parameters.ClimbVelocity * 1.5f, Time.deltaTime * 30f));
                }
    ...
    public void Dash(bool _isFacingRight)
    {
        ...
        State.DashDuration = Parameters.DashDuration;
        dashCharged = false;
